...
class CommentRewardVirtualOperation(Base):
    ...
    __tablename__ = 'dpds_op_virtual_comment_rewards'
    __table_args__ = (
    ...
    operation_type = Column(operation_types_enum,nullable=False,index=True,default='comment_reward')


    _fields = dict(
        payout=lambda x: amount_field(x.get('payout'), num_func=float), # dpay_type:asset
        payout_symbol=lambda x: amount_symbol_field(x.get('payout')), # dpay_type:asset
