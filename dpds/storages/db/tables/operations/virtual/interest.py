...
class InterestVirtualOperation(Base):
    ...
    __tablename__ = 'dpds_op_virtual_interests'
    __table_args__ = (
    ...
    interest = Column(Numeric(20,6), nullable=False) # dpay_type:asset
    interest_symbol = Column(String(5)) # dpay_type:asset
    operation_type = Column(operation_types_enum,nullable=False,index=True,default='interest')


    _fields = dict(
        interest=lambda x: amount_field(x.get('interest'), num_func=float), # dpay_type:asset
        interest_symbol=lambda x: amount_symbol_field(x.get('interest')), # dpay_type:asset
