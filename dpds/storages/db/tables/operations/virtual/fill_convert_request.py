...
class FillConvertRequestVirtualOperation(Base):
    ...
    __tablename__ = 'dpds_op_virtual_fill_convert_requests'
    __table_args__ = (
    ...
    amount_in = Column(Numeric(20,6), nullable=False) # dpay_type:asset
    amount_in_symbol = Column(String(5)) # dpay_type:asset
    amount_out = Column(Numeric(20,6), nullable=False) # dpay_type:asset
    amount_out_symbol = Column(String(5)) # dpay_type:asset
    operation_type = Column(operation_types_enum,nullable=False,index=True,default='fill_convert_request')


    _fields = dict(
        amount_in=lambda x: amount_field(x.get('amount_in'), num_func=float), # dpay_type:asset
        amount_in_symbol=lambda x: amount_symbol_field(x.get('amount_in')), # dpay_type:asset
        amount_out=lambda x: amount_field(x.get('amount_out'), num_func=float), # dpay_type:asset
        amount_out_symbol=lambda x: amount_symbol_field(x.get('amount_out')), # dpay_type:asset
