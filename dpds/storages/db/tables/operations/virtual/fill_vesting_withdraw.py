...
class FillVestingWithdrawVirtualOperation(Base):
    ...
    __tablename__ = 'dpds_op_virtual_fill_vesting_withdraws'
    __table_args__ = (
    ...
    withdrawn = Column(Numeric(20,6), nullable=False) # dpay_type:asset
    withdrawn_symbol = Column(String(5)) # dpay_type:asset
    deposited = Column(Numeric(20,6), nullable=False) # dpay_type:asset
    deposited_symbol = Column(String(5)) # dpay_type:asset
    operation_type = Column(operation_types_enum,nullable=False,index=True,default='fill_vesting_withdraw')


    _fields = dict(
        withdrawn=lambda x: amount_field(x.get('withdrawn'), num_func=float), # dpay_type:asset
        withdrawn_symbol=lambda x: amount_symbol_field(x.get('withdrawn')), # dpay_type:asset
        deposited=lambda x: amount_field(x.get('deposited'), num_func=float), # dpay_type:asset
        deposited_symbol=lambda x: amount_symbol_field(x.get('deposited')), # dpay_type:asset
