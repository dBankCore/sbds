...
class ProducerRewardVirtualOperation(Base):
    ...
    __tablename__ = 'dpds_op_virtual_producer_rewards'
    __table_args__ = (

        ForeignKeyConstraint(['producer'], ['dpds_meta_accounts.name'],
    ...
    producer = Column(String(16)) # dpay_type:account_name_type
    vesting_shares = Column(Numeric(20,6), nullable=False) # dpay_type:asset
    vesting_shares_symbol = Column(String(5)) # dpay_type:asset
    operation_type = Column(operation_types_enum,nullable=False,index=True,default='producer_reward')
