...
class VoteOperation(Base):
    ...
    __tablename__ = 'dpds_op_votes'
    __table_args__ = (
        PrimaryKeyConstraint('block_num', 'transaction_num', 'operation_num'),
        ForeignKeyConstraint(['voter'], ['dpds_meta_accounts.name'],
    ...
    voter = Column(String(16)) # dpay_type:account_name_type
    author = Column(String(16)) # dpay_type:account_name_type
    permlink = Column(Unicode(256), index=True) # name:permlink
    weight = Column(SmallInteger) # dpay_type:int16_t
    operation_type = Column(operation_types_enum,nullable=False,index=True,default='vote')
