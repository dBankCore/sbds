...
class TransferToVestingOperation(Base):
    ...
    __tablename__ = 'dpds_op_transfer_to_vestings'
    __table_args__ = (
    ...
    operation_type = Column(operation_types_enum,nullable=False,index=True,default='transfer_to_vesting')


    _fields = dict(
        amount=lambda x: amount_field(x.get('amount'), num_func=float), # dpay_type:asset
        amount_symbol=lambda x: amount_symbol_field(x.get('amount')), # dpay_type:asset
