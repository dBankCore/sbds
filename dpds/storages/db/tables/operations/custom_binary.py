...
class CustomBinaryOperation(Base):
    ...
    __tablename__ = 'dpds_op_custom_binaries'
    __table_args__ = (
    ...
    required_owner_auths = Column(JSONB) # dpay_type:flat_set< account_name_type>
    required_active_auths = Column(JSONB) # dpay_type:flat_set< account_name_type>
    required_posting_auths = Column(JSONB) # dpay_type:flat_set< account_name_type>
    required_auths = Column(JSONB) # dpay_type:vector< authority>
    id = Column(UnicodeText) # dpay_type:string -> default
    data = Column(String(100)) # dpay_type:vector< char>
    operation_type = Column(operation_types_enum,nullable=False,index=True,default='custom_binary')


    _fields = dict(
        required_owner_auths=lambda x:json_string_field(x.get('required_owner_auths')), # dpay_type:flat_set< account_name_type>
        required_active_auths=lambda x:json_string_field(x.get('required_active_auths')), # dpay_type:flat_set< account_name_type>
        required_posting_auths=lambda x:json_string_field(x.get('required_posting_auths')), # dpay_type:flat_set< account_name_type>
        required_auths=lambda x:json_string_field(x.get('required_auths')), # dpay_type:vector< authority>
