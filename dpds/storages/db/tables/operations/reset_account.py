...
class ResetAccountOperation(Base):
    ...
    __tablename__ = 'dpds_op_reset_accounts'
    __table_args__ = (
        PrimaryKeyConstraint('block_num', 'transaction_num', 'operation_num'),
        ForeignKeyConstraint(['reset_account'], ['dpds_meta_accounts.name'],
            deferrable=True, initially='DEFERRED', use_alter=True),
        ForeignKeyConstraint(['account_to_reset'], ['dpds_meta_accounts.name'],
            deferrable=True, initially='DEFERRED', use_alter=True),)
    ...
    account_to_reset = Column(String(16)) # dpay_type:account_name_type
