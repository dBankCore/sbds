...
class ChangeRecoveryAccountOperation(Base):
    ...
    __tablename__ = 'dpds_op_change_recovery_accounts'
    __table_args__ = (
        PrimaryKeyConstraint('block_num', 'transaction_num', 'operation_num'),
        ForeignKeyConstraint(['account_to_recover'], ['dpds_meta_accounts.name'],
            deferrable=True, initially='DEFERRED', use_alter=True),
        ForeignKeyConstraint(['new_recovery_account'], ['dpds_meta_accounts.name'],
            deferrable=True, initially='DEFERRED', use_alter=True),)
    ...
    new_recovery_account = Column(String(16)) # dpay_type:account_name_type
    extensions = Column(JSONB) # dpay_type:extensions_type
    operation_type = Column(operation_types_enum,nullable=False,index=True,default='change_recovery_account')
