...
class ReportOverProductionOperation(Base):
    ...
    __tablename__ = 'dpds_op_report_over_productions'
    __table_args__ = (
        PrimaryKeyConstraint('block_num', 'transaction_num', 'operation_num'),
        ForeignKeyConstraint(['reporter'], ['dpds_meta_accounts.name'],
    ...
    reporter = Column(String(16)) # dpay_type:account_name_type
    first_block = Column(JSONB) # dpay_type:signed_block_header
    second_block = Column(JSONB) # dpay_type:signed_block_header
    operation_type = Column(operation_types_enum,nullable=False,index=True,default='report_over_production')


    _fields = dict(
        first_block=lambda x:json_string_field(x.get('first_block')), # dpay_type:signed_block_header
        second_block=lambda x:json_string_field(x.get('second_block')), # dpay_type:signed_block_header
