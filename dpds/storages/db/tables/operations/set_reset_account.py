...
class SetResetAccountOperation(Base):
    ...
    __tablename__ = 'dpds_op_set_reset_accounts'
    __table_args__ = (
        PrimaryKeyConstraint('block_num', 'transaction_num', 'operation_num'),
        ForeignKeyConstraint(['account'], ['dpds_meta_accounts.name'],
            deferrable=True, initially='DEFERRED', use_alter=True),
        ForeignKeyConstraint(['current_reset_account'], ['dpds_meta_accounts.name'],
            deferrable=True, initially='DEFERRED', use_alter=True),
        ForeignKeyConstraint(['reset_account'], ['dpds_meta_accounts.name'],
            deferrable=True, initially='DEFERRED', use_alter=True),)
    ...
    current_reset_account = Column(String(16)) # dpay_type:account_name_type
    reset_account = Column(String(16)) # dpay_type:account_name_type
    operation_type = Column(operation_types_enum,nullable=False,index=True,default='set_reset_account')
