...
class AccountWitnessVoteOperation(Base):
    ...
    __tablename__ = 'dpds_op_account_witness_votes'
    __table_args__ = (
        PrimaryKeyConstraint('block_num', 'transaction_num', 'operation_num'),
        ForeignKeyConstraint(['account'], ['dpds_meta_accounts.name'],
            deferrable=True, initially='DEFERRED', use_alter=True),
        ForeignKeyConstraint(['witness'], ['dpds_meta_accounts.name'],
            deferrable=True, initially='DEFERRED', use_alter=True),)
    ...
    witness = Column(String(16)) # dpay_type:account_name_type
    approve = Column(Boolean) # dpay_type:bool
    operation_type = Column(operation_types_enum,nullable=False,index=True,default='account_witness_vote')
