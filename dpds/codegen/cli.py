...
JSONB_TYPES = {
    ...
    'chain_properties',
    'pow',
    'dpay::protocol::pow2_work',
    'pow2_input',
    'fc::equihash::proof',
    'signed_block_header',
    'dpay::protocol::comment_options_extensions_type'
}
...
def get_fields(name, _type):
    ...
    if _type == 'asset':
        ...
        fields.append(
            f"{name}=lambda x: amount_field(x.get('{name}'), num_func=float), # dpay_type:{_type}")
        fields.append(
            f"{name}_symbol=lambda x: amount_symbol_field(x.get('{name}')), # dpay_type:{_type}")

    elif _type == 'time_point_sec':
        fields.append(
            f"{name}=lambda x: dateutil.parser.parse(x.get('{name}')), # dpay_type:{_type}")

    elif _type in JSONB_TYPES:
        fields.append(f"{name}=lambda x:json_string_field(x.get('{name}')), # dpay_type:{_type}")
...
def _get_columns_by_type(name, _type):
    ...
    if _type == 'asset':
        if name == 'from':
            return [
                f'_from = Column("from",Numeric(20,6), nullable=False) # dpay_type:asset',
                f'from_symbol = Column(String(5)) # dpay_type:{_type}']
        return [
            f'{name} = Column(Numeric(20,6), nullable=False) # dpay_type:asset',
            f'{name}_symbol = Column(String(5)) # dpay_type:{_type}']

    # account_name_type
    elif _type == 'account_name_type':
        return [
            f'{name} = Column(String(16),nullable=True) # dpay_type:{_type}']

    # flat_set< account_name_type>
    elif _type == 'flat_set< account_name_type>':
        return [f'{name} = Column(JSONB) # dpay_type:{_type}']

    # public_key_type
    elif _type == 'public_key_type':
        return [f'{name} = Column(String(60), nullable=False) # dpay_type:{_type}']

    # optional< public_key_type>
    elif _type == 'optional< public_key_type>':
        return [f'{name} = Column(String(60)) # dpay_type:{_type}']

    # boolean
    elif _type == 'bool':
        return [f'{name} = Column(Boolean) # dpay_type:{_type}']

    # integers
    elif _type in SMALL_INT_TYPES:
        return [f'{name} = Column(SmallInteger) # dpay_type:{_type}']
    elif _type in INT_TYPES:
        return [f'{name} = Column(Integer) # dpay_type:{_type}']
    elif _type in BIG_INT_TYPES:
        return [f'{name} = Column(Numeric) # dpay_type:{_type}']
    ...
    # vector< char>
        ...
        return [f'{name} = Column(String(100)) # dpay_type:{_type}']

    # block_id_type (fc::ripemd160)
    elif _type == 'block_id_type':
        return [f'{name} = Column(String(40)) # dpay_type:{_type}']
    ...
    # time_point_sec
        ...
        return [f'{name} = Column(DateTime) # dpay_type:{_type}']
    ...
    # dpay::protocol::comment_options_extensions_type
elif _type == 'dpay::protocol::comment_options_extensions_type':
        return [f'{name} = Column(JSONB) # dpay_type:{_type}']
    ...
    # dpay::protocol::pow2_work
elif _type == 'dpay::protocol::pow2_work':
        return [f'{name} = Column(JSONB) # dpay_type:{_type}']
    ...
    # default string
        ...
        return [f'{name} = Column(UnicodeText) # dpay_type:{_type} -> default']
...
def op_table_name(op_name):
    short_op_name = op_name.replace('_operation', '')
    if op_name in VIRTUAL_OPS_NAMES:
        return f'dpds_op_virtual_{INFLECTOR.plural(short_op_name)}'
    return f'dpds_op_{INFLECTOR.plural(short_op_name)}'
...
def _generate_account_refs(headers):
    ...
    block = {
        ...
        'short_name':'block',
        'class_name': 'Block',
        'table_name': 'dpds_core_blocks',
        'field_name': 'witness',
        'class_field_name': 'witness',
        'ref_name':   'dpds_core_blocks.witness',
    }
    refs.append(block)
    for header in headers:
...
@click.group()
def codegen():
    """Generate code to build apps on the dPay Blockchain"""
...
def generate_accounts_view(headers_path, templates_path):
    ...
    for table_name,table_refs in grouped_refs.items():
        if table_name == 'dpds_core_blocks':
            continue
